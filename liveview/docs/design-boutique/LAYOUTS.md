...
| `admin_layout` | `:sidebar`*, `:topbar`*, `:content` | Same full-height nav + header + scrollable main. `immersive` (boolean attr, default `false`) hides the sidebar and topbar below `lg:` and drops `:content`'s padding there, for a consumer that wants to be its own full-height stage (`SiteChrome`'s own `immersive` attr, `GameLive`'s stage mode) |
...
| `game_layout` | `:status`*, `:board`, `:players`*, `:dock`* | **New, LiveView-only** — board region + player panel + live status for turn-based multiplayer (`DESIGN-CONTRACTS.md`'s C5 pending list). No React shape to mirror: designed directly against this doc's slot/grid-area/token contract. `players_first` (boolean attr, default `false`) flips the below-`lg:` stack from `status/board/players` to `status/players/board` — once a game has ended, the outcome and final standings matter more than a board a phone/tablet viewport clips to a fraction of the map; the side-by-side `lg:` arrangement is unaffected either way. `:players` renders once, inside a native `<dialog id="game-drawer">` (`docs/mobile-battle-mode.md` §4.5): a colocated `.Drawer` hook opens it as a right-hand slide-in sheet below `lg:` (a consumer button elsewhere in the tree opens it via `aria-controls="game-drawer"`), and keeps it shown as the plain static rail at `lg:` and up — CSS forces that regardless of the hook, so the rail never depends on JS having mounted. `stage` (boolean attr, default `false`, mobile battle mode WP3, `docs/mobile-battle-mode.md` §4.1) turns the shell into a full-height (`100dvh`, no page scroll) map stage below `lg:`: the grid becomes `status/board/dock`, `:board` drops its own padding/scroll, and `:dock` renders as a `max-h-[45dvh]` bottom sheet (`aria-label="Actions"`) with safe-area insets — the drawer above already stays closed by default at this breakpoint, so `stage` needs no special handling for `:players`. At `lg:` and up `stage` gives the rail column a second row so `:dock` renders once, at its top, above the drawer. |
