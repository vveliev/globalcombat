...
# Idempotent: the dev container runs this on every boot (liveview/Dockerfile.dev), and
# `register_account/1` would fail on the second run once modern_player exists.
if Repo.get_by(Account, name: "modern_player") do
  IO.puts("Seed accounts already present -- skipping (delete modern_player to re-seed).")
else
