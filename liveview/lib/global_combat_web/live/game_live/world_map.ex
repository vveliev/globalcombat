defmodule GlobalCombatWeb.GameLive.WorldMap do
  ...
  sprites the legacy `Web` project still ships (`Web/wwwroot/maps/original/
  <tech><owner>.gif`, nine pre-colored 8-bit tiles per area) with one responsive
  SVG whose territories are `<use>` clones of static `<defs>` outlines, filled
  by CSS from the owner slot.
  ...
  shapes, adjacency and the coordinate space are unchanged from the sprite
  board. The defs are a static template: LiveView ships them once with the page
  statics and never re-sends them on a diff, however often ownership changes.
  ...
  same `owner_slot/1` drives the territory fill, the player-list legend dot and
  the `:elements` sprite suffix, so there is one place the legacy
  `Player.GetColor()` numbering lives.
  """
  ...
  @doc "Whether `map_name` has a vector board; every other map still composites sprites."
  def supports?(:original), do: true
  def supports?(_map_name), do: false

  @doc """
  The colour slot (0..8) for an owner number — the sprite board's
  `owner_number % 9` (`Player.GetColor()` in the original), with `nil` (no
  owner) as slot 0. `--map-owner-N` tokens and the `<tech>N.gif` sprite names
  both follow this numbering.
  """
  ...
  def world_map(assigns) do
    assigns =
      assigns
      |> assign(:view_box, Geometry.view_box())
    ...
    <div class="world-map">
      <svg viewBox={@view_box} role="group" aria-label="World map board" class="block h-auto w-full">
        <.original_map_defs />
        <rect class="world-map-sea" x="0" y="0" width="100%" height="100%" />
        <rect class="world-map-sea-texture" x="0" y="0" width="100%" height="100%" />
  ...
  defp territory(assigns) do
    assigns = assign(assigns, :label, territory_label(assigns.area, assigns.owner_names))
